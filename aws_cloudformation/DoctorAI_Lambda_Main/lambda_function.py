...
def dispatch(intent_request):
    ...
    elif intent_name == 'AskForPathogen':
...
def ask_for_gene(intent_request, conn, db):
    session_attributes = get_session_attributes(intent_request)
...
def ask_infection_organism(intent_request, conn, db):
    ...
    slots = get_slots(intent_request)
    disease_name = get_slot(intent_request, 'disease_name').replace("_", " ")
    ...
    diagnosis_exist = False
    if result:
        text = f"Pathogens that cause {disease_name}: "
        for r in result:
            text += f"{r['p_name']}, "
        diagnosis_exist = True
        
    else:
        text = f"{disease_name} has no known pathogens."
